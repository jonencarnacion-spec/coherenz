...
// ReactNode only exists as a type, not a runtime export — importing it
// as a named value import (as the source component did) breaks Vite's
// dev-mode ESM interop with React's CommonJS build ("Named export
// 'ReactNode' not found"). Type-only import fixes it.
...
const InteractiveSynapseNetwork: React.FC<InteractiveSynapseNetworkProps> = ({
  ...
  return (
    <div
      ...
      // Source used w-screen/h-screen (viewport-root only). Changed to
      // w-full/h-full so it correctly fills whatever container it's placed
      // in (here: the Hero section) instead of the literal viewport.
