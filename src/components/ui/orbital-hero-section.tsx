...
export function OrbitalHeroSection({
  ...
  useEffect(() => {
    ...
    // Source used { alpha: false }, which browsers composite as opaque black
    // (not transparent) until the first draw call runs. Since that first
    // draw happens inside this effect — after the initial paint — there was
    // a real one-frame flash to black instead of the host's navy background
    // showing through. Default (alpha-enabled) canvas starts transparent,
    // so the host's bg-navy shows immediately with no flash.
