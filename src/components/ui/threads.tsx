...
// Ported from React Bits (reactbits.dev) "Threads" component -- the third
// approach-hero background option tried this session (after Galaxy and
// Scanner). Shader/render logic is unmodified from the source; only typed
// for this project's TS conventions.
...
interface ThreadsProps {
  /** RGB triplet, each channel 0-1. Defaults to white. */
...
const Threads = ({
  ...
  useEffect(() => {
    ...
    function resize() {
      ...
      (program.uniforms.iResolution.value as Color).r = gl.canvas.width;
      (program.uniforms.iResolution.value as Color).g = gl.canvas.height;
      (program.uniforms.iResolution.value as Color).b = gl.canvas.width / gl.canvas.height;
    }
